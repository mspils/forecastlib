        ...
        if self.task_name == "long_term_forecast" or self.task_name == "short_term_forecast":
            self.decoders = nn.Sequential(
                *(
                ...
                ResBlock(self.hidden_dim, self.res_hidden, self.decode_dim * self.pred_len, dropout, bias),
            )
            self.temporalDecoder = ResBlock(
                self.decode_dim + self.feature_encode_dim, self.temporalDecoderHidden, 1, dropout, bias
            )
        ...
        if self.task_name == "imputation":
            self.decoders = nn.Sequential(
                *(
                ...
                ResBlock(self.hidden_dim, self.res_hidden, self.decode_dim * self.seq_len, dropout, bias),
            )
            self.temporalDecoder = ResBlock(
                self.decode_dim + self.feature_encode_dim, self.temporalDecoderHidden, 1, dropout, bias
            )
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, batch_y_mark, mask=None):
        ...
        if self.task_name == "long_term_forecast" or self.task_name == "short_term_forecast":
            if batch_y_mark is None:
                batch_y_mark = (
                    torch.zeros((x_enc.shape[0], self.seq_len + self.pred_len, self.feature_dim))
                    .to(x_enc.device)
                    .detach()
                )
            else:

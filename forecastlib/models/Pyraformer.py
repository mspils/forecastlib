...
from forecastlib.layers.Pyraformer_EncDec import Encoder
...
class Model(nn.Module):
    ...
    def __init__(self, configs, window_size=[4, 4], inner_size=5):
        ...
        if self.task_name == "long_term_forecast" or self.task_name == "short_term_forecast":
            ...
            self.projection = nn.Linear((len(window_size) + 1) * self.d_model, configs.enc_in, bias=True)
        elif self.task_name == "classification":
            self.act = torch.nn.functional.gelu
            self.dropout = nn.Dropout(configs.dropout)
            self.projection = nn.Linear((len(window_size) + 1) * self.d_model * configs.seq_len, configs.num_class)

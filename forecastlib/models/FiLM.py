...
class HiPPO_LegT(nn.Module):
    def __init__(self, N, dt=1.0, discretization="bilinear"):
        ...
        self.register_buffer(
            "eval_matrix", torch.Tensor(ss.eval_legendre(np.arange(N)[:, None], 1 - 2 * vals).T).to(device)
        )
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.legts = nn.ModuleList(
            [HiPPO_LegT(N=n, dt=1.0 / self.pred_len / i) for n in self.window_size for i in self.multiscale]
        )
        self.spec_conv_1 = nn.ModuleList(
            [
                SpectralConv1d(
                    in_channels=n, out_channels=n, seq_len=min(self.pred_len, self.seq_len), ratio=configs.ratio
                )

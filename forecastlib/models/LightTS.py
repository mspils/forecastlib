...
class IEBlock(nn.Module):
    ...
    def _build(self):
        self.spatial_proj = nn.Sequential(
            nn.Linear(self.input_dim, self.hid_dim), nn.LeakyReLU(), nn.Linear(self.hid_dim, self.hid_dim // 4)
        )
...
class Model(nn.Module):
    ...
    def _build(self):
        self.layer_1 = IEBlock(
            input_dim=self.chunk_size, hid_dim=self.d_model // 4, output_dim=self.d_model // 4, num_node=self.num_chunks
        )
        ...
        self.layer_3 = IEBlock(
            input_dim=self.d_model // 2, hid_dim=self.d_model // 2, output_dim=self.pred_len, num_node=self.enc_in
        )

...
from forecastlib.layers.DWT_Decomposition import Decomposition
...
class Mixer(nn.Module):
    def __init__(
        self, input_seq=[], out_seq=[], batch_size=[], channel=[], d_model=[], dropout=[], tfactor=[], dfactor=[]
    ):
        super().__init__()
        self.input_seq = input_seq
...
class ResolutionBranch(nn.Module):
    def __init__(
        ...
        self.head = nn.Sequential(
            nn.Flatten(start_dim=-2, end_dim=-1), nn.Linear(self.patch_num * self.d_model, self.pred_seq)
        )

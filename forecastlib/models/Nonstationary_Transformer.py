...
from forecastlib.layers.SelfAttention_Family import AttentionLayer, DSAttention
from forecastlib.layers.Transformer_EncDec import Decoder, DecoderLayer, Encoder, EncoderLayer
...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        x_dec_new = (
            torch.cat([x_enc[:, -self.label_len :, :], torch.zeros_like(x_dec[:, -self.pred_len :, :])], dim=1)
            .to(x_enc.device)
            .clone()
        )

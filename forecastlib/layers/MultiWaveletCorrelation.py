...
class FourierCrossAttentionW(nn.Module):
    def __init__(
        self, in_channels, out_channels, seq_len_q, seq_len_kv, modes=16, activation="tanh", mode_select_method="random"
    ):
        super().__init__()
...
class sparseKernelFT1d(nn.Module):
    ...
    def forward(self, x):
        ...
        la = min(self.modes1, N // 2 + 1)
        out_ft = torch.zeros(B, c * k, N // 2 + 1, device=x.device, dtype=torch.cfloat)
        out_ft[:, :, :la] = self.compl_mul1d(
            "bix,iox->box", x_fft[:, :, :la], torch.complex(self.weights1, self.weights2)[:, :, :la]
        )

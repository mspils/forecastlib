...
class Decomposition(nn.Module):
    def __init__(
        ...
        # This lead to tensors on different devices during model initilization when not using gpu 0
        ...
        self.dwt = (
            DWT1DForward(wave=self.wavelet_name, J=self.level, use_amp=self.use_amp).to(self.device)
            if self.device.type == "cuda"
            else DWT1DForward(wave=self.wavelet_name, J=self.level, use_amp=self.use_amp)
        )
        self.idwt = (
            DWT1DInverse(wave=self.wavelet_name, use_amp=self.use_amp).to(self.device)
            if self.device.type == "cuda"
            else DWT1DInverse(wave=self.wavelet_name, use_amp=self.use_amp)
        )

        self.input_w_dim = (
            self._dummy_forward(self.input_length) if not self.no_decomposition else [self.input_length]
        )  # length of the input seq after decompose
        self.pred_w_dim = (
            self._dummy_forward(self.pred_length) if not self.no_decomposition else [self.pred_length]
    ...
    def _dummy_forward(self, input_length):
        ...
        la = []
        la.append(yl.shape[-1])
        for i in range(len(yh)):
            la.append(yh[i].shape[-1])
        return la
...
def mypad(x, pad, mode="constant", value=0):
    ...
    if mode == "symmetric":
        ...
        if pad[0] == 0 and pad[1] == 0:
            m1, m2 = pad[2], pad[3]
            l1 = x.shape[-2]
            xe = reflect(np.arange(-m1, l1 + m2, dtype="int32"), -0.5, l1 - 0.5)
            return x[:, :, xe]
        # horizontal only
        if pad[2] == 0 and pad[3] == 0:
            m1, m2 = pad[0], pad[1]
            l2 = x.shape[-1]
            xe = reflect(np.arange(-m1, l2 + m2, dtype="int32"), -0.5, l2 - 0.5)
            return x[:, :, :, xe]
        # Both
        m1, m2 = pad[0], pad[1]
        l1 = x.shape[-1]
...
def sfb1d(lo, hi, g0, g1, use_amp, mode="zero", dim=-1):
    ...
    elif mode == "zero" or mode == "symmetric" or mode == "reflect" or mode == "periodic":
...
def afb2d_nonsep(x, filts, mode="zero"):
    ...
    if isinstance(filts, (tuple, list)):
        if len(filts) == 2:
            ...
            filts = prep_filt_afb2d_nonsep(filts[0], filts[1], filts[2], filts[3], device=x.device)
...
def sfb2d_nonsep(coeffs, filts, mode="zero"):
    ...
    if isinstance(filts, (tuple, list)):
        if len(filts) == 2:
            ...
            filts = prep_filt_sfb2d_nonsep(filts[0], filts[1], filts[2], filts[3], device=coeffs.device)
        else:
...
def prep_filt_afb2d_nonsep(h0_col, h1_col, h0_row=None, h1_row=None, device=None):
    ...
    filts = np.stack([ll[None, ::-1, ::-1], lh[None, ::-1, ::-1], hl[None, ::-1, ::-1], hh[None, ::-1, ::-1]], axis=0)

    ...
    def forward(self, q, k, v, mask):
        ...
        for wi, i in enumerate(self.index):
            ...
            out_ft[:, :, :, wi] = self.compl_mul1d(
                "bhi,hio->bho", x_ft[:, :, :, i], torch.complex(self.weights1, self.weights2)[:, :, :, wi]
            )
...
class FourierCrossAttention(nn.Module):
    def __init__(
        ...
        self.weights1 = nn.Parameter(
            self.scale
            * torch.rand(
                num_heads, in_channels // num_heads, out_channels // num_heads, len(self.index_q), dtype=torch.float
            )
        )
        self.weights2 = nn.Parameter(
            self.scale

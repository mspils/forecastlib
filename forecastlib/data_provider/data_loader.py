...
from forecastlib.utils.timefeatures import get_data_stamp, time_features
...
class Dataset_Custom(BaseForecastDataset):
    ...
    def __read_data__(self):
        ...
        if self.diff:
            # M: difference every column; MS/S: only the target. The *_d1 columns are appended at the end.
            diff_cols = list(df_data.columns) if self.features == "M" else [self.target]
            df_diff = df_data[diff_cols].diff()
            df_diff.iloc[0] = 0
            df_diff.columns = [f"{c}_d1" for c in diff_cols]
            df_data = pd.concat([df_data, df_diff], axis=1)
    ...
    def __getitem__(self, index):
        ...
        if self.diff:
            # The first difference in the window refers to a step outside of it, so it is zeroed.
            # Copy first, seq_x is a view into self.data_x.
            seq_x = seq_x.copy()
            if self.features == "M":
